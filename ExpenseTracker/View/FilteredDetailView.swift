        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 15) {
                HStack(spacing: 15) {
                    ...
                    Button {
                        expenseViewModel.showFilterView.toggle()
                    } label: {
        ...
        ZStack {
            ...
            if expenseViewModel.showFilterView {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    DatePicker("", selection: $expenseViewModel.startDate, in:
                    ...
                    DatePicker("", selection: $expenseViewModel.endDate, in:
                                Date.distantPast...Date(), displayedComponents: [.date])
                    .labelsHidden()
                    .datePickerStyle(.compact)
                }
        ...
        HStack(spacing: 0) {
            ForEach([ExpenseType.income, ExpenseType.expense], id: \.rawValue) { tab in
                Text(tab.rawValue.capitalized)
                    .fontWeight(.semibold)
                    .foregroundStyle(expenseViewModel.tabName == tab ? .white : .black)
